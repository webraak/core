...
namespace webraak\boot;

define('WEBRAAK_CORE_PATH', WEBRAAK_PATH . 'pincore' . DIRECTORY_SEPARATOR);
define('WEBRAAK_BOOT_PATH', WEBRAAK_CORE_PATH . 'boot' . DIRECTORY_SEPARATOR);
define('WEBRAAK_MODEL_PATH', WEBRAAK_CORE_PATH . 'model' . DIRECTORY_SEPARATOR);
define('WEBRAAK_COMPONENT_PATH', WEBRAAK_CORE_PATH . 'component' . DIRECTORY_SEPARATOR);
define('WEBRAAK_SERVICE_PATH', WEBRAAK_CORE_PATH . 'service' . DIRECTORY_SEPARATOR);
define('WEBRAAK_CONFIG_PATH', WEBRAAK_CORE_PATH . 'config' . DIRECTORY_SEPARATOR);
define('WEBRAAK_LANG_PATH', WEBRAAK_CORE_PATH . 'lang' . DIRECTORY_SEPARATOR);
define('WEBRAAK_PATH_THUMB', 'thumbs/{name}_{size}.{ext}');
...
class Loader
{
    ...
    {
        include WEBRAAK_BOOT_PATH . 'functions.php';
    }
    ...
    private static function load($class)
    {
        ...
        if ($countArrayName < 3 || $arrayName[0] != 'webraak') return;
    ...
    private static function modelLoader()
    {
        ...
        self::includeClass(WEBRAAK_MODEL_PATH, $className, 'model');
    }
    ...
    private static function loadMagicsMethodsTrait($className)
    {
        ...
        if (in_array("webraak\\component\\MagicTrait", $uses)) {
    ...
    private static function databaseLoader()
    {
        ...
        self::includeClass(WEBRAAK_MODEL_PATH, $className, 'database');
    }
    ...
    {
        $path = ($app === '~') ? WEBRAAK_COMPONENT_PATH : Dir::path('component/', $app);
    ...
    private static function serviceLoader()
    {
        ...
        self::includeClass(WEBRAAK_SERVICE_PATH, $className, 'service');
    }
    ...
    {
        $path = ($app === '~') ? WEBRAAK_COMPONENT_PATH : Dir::path('service/', $app);
        $className = lcfirst(str_replace('Service', '', self::$className));
