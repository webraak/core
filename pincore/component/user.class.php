...
class User
{
    ...
    public static function getTokenKey()
    {
        ...
        switch (self::$type) {
            case self::COOKIE:
                self::$token_key = Cookie::get('webraak_user');
                break;
            case self::JWT:
                self::$token_key = self::authToken();
                break;
            case self::SESSION:
                self::$token_key = Session::get('webraak_user');
                break;
        }
    ...
    public static function authToken($token = null)
    {
        ...
        try {
            ...
            $token_key = $payload->webraak_user;
    ...
    public static function setToken($data)
    {
        ...
        $token_key = self::getTokenKey();
        $token_key = Token::generate($data, 'webraak_user', $user_id, $token_key);
    ...
    private static function setClientToken($token_key)
    {
        ...
        switch (self::$type) {
            case self::COOKIE:
                Cookie::set('webraak_user', $token_key, 999999999);
                break;
            case self::JWT:
                $payloadArray = [
                    'webraak_user' => $token_key,
                ];
                self::$login_key = JWT::encode($payloadArray, self::$secret_key);
                break;
            case self::SESSION:
                Session::lifeTime(999999999);
                Session::set('webraak_user', $token_key);
                break;
        }
    }
    ...
    private static function removeToken()
    {
        ...
        if (!TokenModel::fetch_by_key_without_app($token_key)) {
            switch (self::$type) {
                case self::COOKIE:
                    Cookie::destroy('webraak_user');
                    break;
                case self::SESSION:
                    Session::remove('webraak_user');
                    if (Session::has())
