...
class DB
{
     ...
     */
    public function __construct($host = null, $username = null, $password = null, $db = null, $prefix = null, $port = null, $charset = 'utf8')
    {
        $isSubQuery = false;
    ...
    {
        if (!empty($this->_limit)) $numRows = $this->_limit;
        ...
        if (!is_object($tableName)) {
            ...
            $this->_tableName = '(' . $tableName->_lastQuery . ') ' . $tableName->host;
        }
    ...
    public function whereWithNull($whereProp, $whereValue)
    {
        if (is_null($whereValue))
            $this->where($whereProp, null, 'IS');
        else
            $this->where($whereProp, $whereValue);
    }

    public function whereInWithNull($whereProp, $whereValue)
    {
        if (!is_array($whereValue)) return;

        $arr = array_values(array_filter($whereValue));

        if (!empty($arr)) {
            $this->pOpen();
            $this->where($whereProp, $arr, 'IN');
            if (in_array(null, $whereValue))
                $this->orWhere($whereProp, null, 'IS');
            $this->pClose();
        } else {
            if (in_array(null, $whereValue))
                $this->where($whereProp, null, 'IS');
    ...
    {
        if (empty($this->_where)) $this->isParenthesis = true;
        $this->_where[] = '(';
    }
