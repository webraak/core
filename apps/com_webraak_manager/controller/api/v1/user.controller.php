...
use webraak\component\Uploader;
...
class UserController extends MasterConfiguration
{
    ...
    public function deleteAvatar()
    {
        if (!User::isLoggedIn())
            $this->error();
        $user = User::get();
        Uploader::init()->thumb('128f', WEBRAAK_PATH_THUMB)->actRemoveRow($user['avatar_id']);
    ...
    public function changeAvatar()
    {
        ...
        if (Request::isFile('avatar')) {
            ...
            if ($up->isCommit() && UserModel::update_avatar(User::get('user_id'), $avatar_id)) {
                ...
                $up->commit();
                Uploader::init()->thumb('128f', WEBRAAK_PATH_THUMB)->actRemoveRow($old_avatar_id);
            }
            if ($result = $up->result()) {
                Response::json([
