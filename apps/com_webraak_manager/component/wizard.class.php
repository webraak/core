...
use webraak\component\Zip;
use webraak\model\WebraakDatabase;
use webraak\model\SessionModel;
...
class Wizard
{
    ...
    public static function installApp($pinFile)
    {
        ...
        self::setApp('com_webraak_manager', true);
    ...
    private static function removeDatabase($packageName)
    {
        ...
        $tables = WebraakDatabase::getTables($packageName);
        $tables = implode(',', $tables);
        WebraakDatabase::$db->rawQuery("SET FOREIGN_KEY_CHECKS = 0");

        //delete all tables
        if (!empty($tables))
            WebraakDatabase::$db->rawQuery("DROP TABLE IF EXISTS " . $tables);
        ...
        WebraakDatabase::$db->rawQuery("SET FOREIGN_KEY_CHECKS = 1");
